...
int agentstate[N], agentstatenext[N];
double r, etaI, lambda, Activity, InfectedOverATrial, InfectedOverTrials;
double AvgInfectedEtaLambda[LambdaSize], a[N];
double results[3000];
...
int main()
{
	...
	for (i_time = 0; i_time < Tmax; i_time++) //cycle on the duration of a trial
	{
		...
		for (i = 0; i < N; i++) //copy the old agent state in the new one
		{
			agentstatenext[i] = agentstate[i];
		}

		for (i = 0; i < N; i++) //cycle on agents - epidemics on network
		{
			...
			{
				for (j = 0; j < M; j++) //cycle on the neighbors of the active agent
				{
		...
		NumInf = 0;
		for (i = 0; i < N; i++) //copy new state into the current state
		{

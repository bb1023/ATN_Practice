...
@everywhere begin
    ...
    tmax = 200         # maximum duration of the simulation
    Ni = Int(0.01 * N)  # initial number of infects
    gamma = -2.1        # exponent of the power law
    NTrials = 3        # number of trials for each value of lambda
    TWindow = 20       # time window for evaluating steady state
    LambdaSize = 1000
    LambdaMin = 0.0
    LambdaMax = 1.0
    LambdaStep = 0.05
...
AvgInfectedEtaLambda = @showprogress pmap(atn_module.avg_infected_par,
                                          N * ones(Int, LambdaSize, EtaSize),
                                          NTrials * ones(Int, LambdaSize, EtaSize),
                                          m * ones(Int, LambdaSize, EtaSize),
                                          mu * ones(LambdaSize, EtaSize), grid,
                                          etaS * ones(LambdaSize, EtaSize), Activity,
...
matwrite(mat_name, Dict("AvgInfectedEtaLambda" => AvgInfectedEtaLambda); compress=true)
# AvgInfectedEtaLambda = Float64.(transpose(AvgInfectedEtaLambda))
mat"figure();set(gcf, 'Position',  [0, 0, 2500, 1500]); hold on;"
mat"axis([ -0,1,-0,10 ])"
mat"imagesc([0,1],[0,10],$AvgInfectedEtaLambda)"nfectedEtaLambda))
mat"figure();set(gcf, 'Position',  [0, 0, 2500, 1500]); hold on;"
mat"axis([ -0,1,-0,10 ])"
